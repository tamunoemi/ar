...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		// initialize data
		$this->identity_column = $this->config->item('email', 'ion_auth');
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code !== FALSE)
		{
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['login'], $data, array('id' => $id));
		}
		else
		{
			$data = array(
	...
	public function clear_forgotten_password_code($code) {
		...
		if ($this->db->count_all_results($this->tables['login']) > 0)
		{
			$data = array(
			...
			$this->db->update($this->tables['login'], $data, array('forgotten_password_code' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['login'], $data, array($this->identity_column => $identity));
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_password_in_db = $this->db->update($this->tables['login'], $data, array($this->identity_column => $identity));
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where('username', $username)
						->limit(1)
						->count_all_results($this->tables['login']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->db->update($this->tables['login'], $update, array($this->identity_column => $identity));
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$user_data = array_merge($this->_filter_data($this->tables['login'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['login'], $user_data);

		$id = $this->db->insert_id($this->tables['login'] . '_id_seq');
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['login'].'.*',
			    $this->tables['login'].'.id as id',
			    $this->tables['login'].'.id as user_id'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['users_groups'].'.'.$this->join['login'].'='.$this->tables['login'].'.id',
				    'inner'
				);
			}
		...
		$this->response = $this->db->get($this->tables['login']);
	...
	public function user($id = NULL)
	{
		...
		$this->limit(1);
		$this->order_by($this->tables['login'].'.id', 'desc');
		$this->where($this->tables['login'].'.id', $id);
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
		                ->where($this->tables['users_groups'].'.'.$this->join['login'], $id)
	...
	public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
	{
		...
		if (!empty($group_ids))
		{
			...
			foreach ($group_ids as $group_id)
			{
				...
				$this->db->delete(
					$this->tables['users_groups'],
					array($this->join['groups'] => (float)$group_id, $this->join['login'] => (float)$user_id)
				);
		...
		{
			// Cast to float to support bigint data type
			if ($return = $this->db->delete($this->tables['users_groups'], array($this->join['login'] => (float)$user_id)))
			{
	...
	public function update($id, array $data)
	{
		...
		$data = $this->_filter_data($this->tables['login'], $data);

		if (array_key_exists($this->identity_column, $data) || array_key_exists('password', $data) || array_key_exists('username', $data))
		{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['login'], $data, array('id' => $user->id));
	...
	public function delete_user($id)
	{
		...
		$this->db->delete($this->tables['login'], array('id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['login'], array('last_login' => time()), array('id' => $id));
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['login'], array('remember_code' => $salt), array('id' => $id));
