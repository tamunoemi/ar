...
if($pass=='' || $email=='')
{
	...
	if($redirect_to=='') header("Location: ".get_app_info('path')."/index.php/auth/login?e=1");
	else header("Location: ".get_app_info('path')."/index.php/auth/login?e=1&redirect=$redirect_to");
	exit;
}
else
{
	...
	if ($r && mysqli_num_rows($r) > 0)
	{
		...
		if($auth_enabled)
		{
			...
			if($tied_to=='')
			{
				if($redirect_to=='') header("Location: ".get_app_info('path').'/index.php/auth/two-factor');
				else header("Location: ".get_app_info('path').'/index.php/auth/two-factor?redirect='.$redirect_to);
			}
			else
			{
				if($redirect_to=='') header("Location: ".get_app_info('path')."/index.php/auth/two-factor?redirect=app?i=".$_SESSION['restricted_to_app']);
				else header("Location: ".get_app_info('path')."/index.php/auth/two-factor?redirect=".$redirect_to);
			}
		}	
